    ...
    <div class="flex items-center space-x-3">
        ...
        <div x-data="{ open: false }" class="relative">
            ...
            <div x-show="open" @click.away="open = false" x-cloak class="absolute right-0 mt-2 w-48 bg-white rounded-lg shadow-xl border border-luxury-border py-2 z-10">
                <a href="{{ route('export.guests.csv') }}" class="block px-4 py-2 text-sm text-gray-700 hover:bg-luxury-bg flex items-center space-x-2">
...
<div class="mb-6 bg-white rounded-xl border border-luxury-border p-4">
    <form action="{{ route('guests.index') }}" method="GET" class="flex flex-col md:flex-row gap-4">
        <div class="flex-1">
            <div class="relative">
                <input type="text" 
                       name="search" 
        ...
        <div class="flex gap-2">
            ...
            @if($query)
            <a href="{{ route('guests.index') }}" class="px-6 py-2 border border-luxury-border text-gray-700 rounded-lg hover:bg-luxury-bg transition-colors font-medium">
                Clear
            </a>
            @endif
        </div>
