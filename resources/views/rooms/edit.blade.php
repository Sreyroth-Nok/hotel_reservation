...
@section('title', 'Edit Room - Hotel Reservation System')
...
<div class="max-w-3xl mx-auto">
    <div class="bg-white rounded-2xl border border-luxury-border overflow-hidden shadow-lg">
        ...
        <div class="px-8 py-6 border-b border-luxury-border bg-gradient-to-r from-primary to-primary-dark">
            <h2 class="text-2xl font-serif font-bold text-white">Edit Room - {{ $room->room_number }}</h2>
            <p class="text-gray-300 mt-1">Update room information and status</p>
        </div>

        <form action="{{ route('rooms.update', $room->room_id) }}" method="POST" class="p-8">
            @csrf
            @method('PUT')
                    ...
                    <div>
                        ...
                        <select name="type_id" 
                            ...
                            @foreach($room_types ?? [] as $type)
                                <option value="{{ $type->type_id }}" {{ old('type_id', $room->type_id) == $type->type_id ? 'selected' : '' }}>
            ...
            <div class="mb-8">
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                    <label class="relative flex cursor-pointer rounded-lg border border-luxury-border p-5 hover:border-accent transition-colors">
                        <input type="radio" name="status" value="available" {{ old('status', $room->status) === 'available' ? 'checked' : '' }} class="sr-only peer" required>
                    ...
                    <label class="relative flex cursor-pointer rounded-lg border border-luxury-border p-5 hover:border-accent transition-colors">
                        <input type="radio" name="status" value="occupied" {{ old('status', $room->status) === 'occupied' ? 'checked' : '' }} class="sr-only peer">
                        <div class="flex items-start space-x-3 w-full">
                    ...
                    <label class="relative flex cursor-pointer rounded-lg border border-luxury-border p-5 hover:border-accent transition-colors">
                        <input type="radio" name="status" value="maintenance" {{ old('status', $room->status) === 'maintenance' ? 'checked' : '' }} class="sr-only peer">
                        <div class="flex items-start space-x-3 w-full">
